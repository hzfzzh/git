...
func (this *Server) Handler(conn net.Conn) { //形参即是当前创建成功的链接的套接字
	...
	//当前handler阻塞
	for {
		select { //这里要理解select阻塞机制，不同于switch，当未进入isLive里，即10s后没任何操作，就超时，否则循环select，重置10s
		//这里两个管道，分别监听，如果isLive触发，说明活跃，重新select重置定时器，如果time超时触发，则说明超时
		case <-isLive:
			//当前用户是活跃的，应该重置定时器，不做任何事情，为了激活select，更新下面的计时器
		case <-time.After(time.Second * 300):
			//说明已经超时，将当前User强制的关闭
			user.SendMsg("YOU ARE offline without alive你被踢了")
			//销毁用的资源
			close(user.C)
			//关闭连接
			conn.Close()
